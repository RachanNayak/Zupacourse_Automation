"""Login helper for Divyakala LMS (email + password or email + OTP)."""
import os
...
def sign_out(page: Page, base_url: Optional[str] = None) -> None:
    ...
    url = (base_url or "").rstrip("/") or os.getenv("BASE_URL", "https://app.sandbox.lms.zupaloop.ai").rstrip("/")
...
def login_as_admin(
    ...
    *,
    ...
    # Email step: fill Email ID and click Continue
    email_input = (
    ...
    password_val = password or os.getenv("ADMIN_PASSWORD")
    otp_val = os.getenv("TEST_OTP")
    ...
    if password_val:
        ...
        raise ValueError(
            "Login requires ADMIN_PASSWORD or TEST_OTP in .env. "
            "Copy .env.example to .env and set ADMIN_PASSWORD for admin (rachan@zupaloop.com)."
        )
...
def _login_manual_otp(page: Page, wait_for_navigation: bool) -> None:
    ...
    timeout_ms = int(os.getenv("MANUAL_OTP_WAIT_SECONDS", "90")) * 1000
